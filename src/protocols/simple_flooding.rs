...
impl<T: Payload + Default + Hash + Eq> Command for StartSimpleFlooding<T> {
    fn execute(&self, sim: &mut Simulation) -> Result<(), Box<dyn Error>> {
        flood(&mut sim.node_interface(self.0), self.1.clone());
        Ok(())
    }
}
...
pub fn flood<T: Payload + Default + Hash + Eq>(node: &mut NodeInterface, message: T) {
...
mod tests {
    ...
    fn simple_flooding_floods() {
        ...
        flood(&mut sim.node_interface(start_node), flooded_value);
